...
namespace mwse::openmw::host {
	...
	int Host::interfaceIndexThunk(lua_State* state) {
		...
		if(requester->container=="PLAYER"&&std::string_view(key,size)=="SkillProgression") { ++host->mInterfaceLookups; host->pushSkillProgressionInterface(id); return 1; }
	...
	void Host::pushBuiltinPackage(ScriptInstance& instance, std::string_view name) {
		if(name=="openmw.util")pushUtilPackage(instance);else if(name=="openmw.interfaces")pushInterfacesPackage(instance);else if(name=="openmw.async")pushAsyncPackage(instance);else if(name=="openmw.storage")pushStoragePackage(instance);else if(name=="openmw.core")pushCorePackage(instance);else if(name=="openmw.self")pushSelfPackage(instance);else if(name=="openmw.types")pushTypesPackage(instance);else if(name=="openmw.input")pushInputPackage(instance);
		else if(name=="openmw.compatibility"){lua_newtable(mLua);lua_pushinteger(mLua,OpenMWApiRevision);lua_setfield(mLua,-2,"apiRevision");lua_pushinteger(mLua,BridgeVersion);lua_setfield(mLua,-2,"bridgeVersion");lua_pushinteger(mLua,static_cast<lua_Integer>(mRuntimeGeneration));lua_setfield(mLua,-2,"runtimeGeneration");lua_pushboolean(mLua,1);lua_setfield(mLua,-2,"foundationPackagesAvailable");lua_pushboolean(mLua,1);lua_setfield(mLua,-2,"gameplayBindingsAvailable");lua_pushboolean(mLua,1);lua_setfield(mLua,-2,"inputEngineHandlersAvailable");lua_pushlightuserdata(mLua,this);lua_pushcclosure(mLua,&compatibilityRecordProbeThunk,1);lua_setfield(mLua,-2,"recordFoundationProbe");if((mConfig.flags&InitializationHarnessMode)!=0){lua_pushlightuserdata(mLua,this);lua_pushcclosure(mLua,&compatibilityInvalidHandleProbeThunk,1);lua_setfield(mLua,-2,"probeInvalidHandles");lua_pushlightuserdata(mLua,this);lua_pushcclosure(mLua,&compatibilityActionUpdateThunk,1);lua_setfield(mLua,-2,"updateAction");lua_pushlightuserdata(mLua,this);lua_pushcclosure(mLua,&compatibilityNativeEventThunk,1);lua_setfield(mLua,-2,"queueNativeEvent");}pushReadOnlyProxy(-1,false);lua_remove(mLua,-2);}
